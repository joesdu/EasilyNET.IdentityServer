...
import { getApiScopes, getApiScope, createApiScope, updateApiScope, deleteApiScope, ApiScope, CreateApiScopeRequest } from '@/services/api';
...
export default function ApiScopesPage() {
  ...
  const handleEdit = useCallback(async (record: ApiScope) => {
    setEditingId(record.id);
    const fullScope = await getApiScope(record.id);
    form.setFieldsValue({
      ...fullScope,
    });
    setModalOpen(true);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const payload: CreateApiScopeRequest = {
      name: values.name,
    ...
    if (editingId) {
      await updateApiScope(editingId, payload);
      message.success('更新成功');
    } else {
      await createApiScope(payload);
  ...
  const columns = [
    ...
    {
      title: '强调', dataIndex: 'emphasize', key: 'emphasize', width: 70,
    ...
    },
    {
      ...
      render: (_: unknown, record: ApiScope) => (
        <Space size="small">
  ...
  return (
    <PageContainer>
      ...
      <Table columns={columns} dataSource={scopes} loading={loading} rowKey="id" pagination={{ pageSize: 10 }} />

      <Modal
        title={editingId ? '编辑 API 作用域' : '新建 API 作用域'}
        open={modalOpen}
        onOk={handleSubmit}
      ...
      >
        <Form form={form} layout="vertical" size="middle">
          {!editingId && (
            <Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入作用域名称' }]}>
          ...
          <Card title="基本信息" size="small">
            ...
            <Form.Item name="description" label="描述">
              <Input.TextArea rows={2} placeholder="作用域描述信息" />
            </Form.Item>
          </Card>
